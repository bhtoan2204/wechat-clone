...
func (h *searchUsersHandler) Handle(c *gin.Context) (interface{}, error) {
	ctx := c.Request.Context()
	logger := logging.FromContext(ctx)
	var request in.SearchUsersRequest
	...
	result, err := h.searchUsers.Dispatch(ctx, &request)
	if err != nil {
		logger.Errorw("SearchUsers failed", zap.Error(err))
		return nil, stackErr.Error(err)
	}
	return result, nil
}
