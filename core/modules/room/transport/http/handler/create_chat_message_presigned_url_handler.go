...
func NewCreateChatMessagePresignedURLHandler(
	createChatMessagePresignedURL cqrs.Dispatcher[*in.CreateChatMessagePresignedURLRequest, *out.CreateChatMessagePresignedURLResponse],
) *createChatMessagePresignedURLHandler {
	return &createChatMessagePresignedURLHandler{
		createChatMessagePresignedURL: createChatMessagePresignedURL,
	}
}

func (h *createChatMessagePresignedURLHandler) Handle(c *gin.Context) (interface{}, error) {
	ctx := c.Request.Context()
	logger := logging.FromContext(ctx)
	var request in.CreateChatMessagePresignedURLRequest
	...
	result, err := h.createChatMessagePresignedURL.Dispatch(ctx, &request)
	if err != nil {
		logger.Errorw("CreateChatMessagePresignedURL failed", zap.Error(err))
		return nil, stackErr.Error(err)
	}
	return result, nil
}
